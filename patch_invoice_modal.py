...
content = content.replace(
    '<td>${tx.inv}</td>',
    '<td><a href="#" onclick="showInvoiceDetail(\\'${tx.inv}\\'); return false;" style="color:var(--accent);text-decoration:underline;">${tx.inv}</a></td>'
)

# And similarly in renderHistory if needed (though requirement specifically mentions "30 Transaksi Terakhir" in Akuntansi table)
# "txListTable" uses similar rendering. Let's patch `renderTxListTable` specifically if it exists, or just do it globally for tx.inv
...
function showInvoiceDetail(inv) {
  ...
  const diskon = (total + ongkir) - grand; // Approx diskon if not explicitly stored

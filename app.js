...
function _renderProductsByKategori(products){
  ...
  const filtered=products.filter(p=>{
    ...
    const matchSearch=!searchText||name.toLowerCase().includes(searchText)||(p.sku||"").toLowerCase().includes(searchText);
...
window.App = {
  ...
  ,reduceStockForTx,
  addQuickCash, setUangPas, calcKembalian, numpadPress
};
...
async function _posRenderGrid(){
  ...
  const filtered = products.filter(p=>{
    ...
    return (_posKategori==="Semua"||k===_posKategori) && (!q||n.includes(q)||(p.sku||"").toLowerCase().includes(q));
  });

...
function generatePaymentNonce(){
  ...
  return 100 + (((arr[0] << 8) | arr[1]) % 900); // 100â€“999
}

// Signature 8-char dari payload transaksi â€” device-bound
...
/* ================= LIMIT (canonical â€” Core is source of truth) ================= */
function getLimit(){
...
function initMode(){
  ...
  if(quickCheck){
    ...
    // Background verify â€” destroy session if token is invalid
...
// ðŸ” Device-bound session destroy
...
// ðŸ” Device-bound session create (async)
...
// Tap-5 to open pin â€” guarded so DOM is ready
...
async function checkPin(){
  if(Date.now()<lockUntil){ document.getElementById("pinError").innerText="â›” Terlalu banyak percobaan. Tunggu 30 detik"; return; }
  ...
  if(window.Core && Core.verifyPin){
    if(Core.isPinLocked()){
      ...
      document.getElementById("pinError").innerText="â›” Terlalu banyak percobaan. Tunggu "+rem+"s";
      return;
    }
    ...
    if(ok){
      ...
      failedAttempts=0; showToast("âœ… Login berhasil");
    } else {
      Core.recordPinFail();
      failedAttempts++;
      document.getElementById("pinError").innerText="âŒ PIN salah";
      if(Core.isPinLocked()) document.getElementById("pinError").innerText="â›” Terlalu banyak percobaan. Tunggu "+Core.getPinLockRemaining()+"s";
      pinBuffer=""; updatePinDots();
    }
    return;
  }
  ...
  if(!savedHash&&!isFirstLogin()){ document.getElementById("pinError").innerText="âš ï¸ PIN belum diset."; return; }
  if(isFirstLogin()){
...
const products = ensureProductIndex(getProducts());
  ...
  cart.forEach(i=>{
    const real = products._map[i.id]; // âš¡ O(1)
    if(!real) return;
...
async function saveTransaction(cart, data, extraFields={}){
  ...
  // Stock reduce â€” persist updated products
  if(window.Admin && Admin.saveProducts){
    Admin.saveProducts(getProducts());
    Admin.invalidateProductCache && Admin.invalidateProductCache();
  }

  // ðŸ” SIGNATURE â€” anti tamper
...
if(!item && delta > 0){
  ...
  // ðŸ” TAMBAHAN INI
  if(!p || p._invalid){
    showToast("âŒ Produk tidak valid");
    return;
  }
...
setTimeout(()=>{
  ...
  if(data.bundleActive && !lastBundle) showToast("ðŸ”¥ Bundle aktif!");
  if(data.freeOngkir && !lastOngkir)  showToast("ðŸšš Free ongkir aktif!");
...
function renderHeroPromo(){
  ...
  // Promo 1
  if(promo1El){
    promo1El.style.backgroundImage = leftImages[0]
      ? `linear-gradient(130deg, rgba(7,12,25,.75), rgba(7,12,25,.4)), url(${leftImages[0]})`
      : "";
  }

  // Promo 2
  if(promo2El){
    promo2El.style.backgroundImage = rightImages[0]
      ? `linear-gradient(130deg, rgba(7,12,25,.75), rgba(7,12,25,.4)), url(${rightImages[0]})`
      : "";
  }
...
function _renderProductsByKategori(products){
  ...
  if(!filtered.length){
    el.innerHTML=`<div class="empty-state"><div class="empty-icon">ðŸ”</div><div>Tidak ada produk ditemukan</div></div>`;
    return;
  }
  ...
  function buildChunk(){
    ...
    while(katIdx<katKeys.length && count<RENDER_CHUNK){
      ...
      while(cardIdx<cards.length && count<RENDER_CHUNK){
        ...
        cardEl.outerHTML; // noop â€” we set innerHTML below
...
/* â”€â”€â”€ Targeted card update (avoid full redraw on qty change) â”€â”€â”€ */
...
function _productCardHTML(p){
  ...
  const imgHtml = img
    ...
    : `<div class="product-card-no-img">ðŸ›ï¸</div>`;
...
/* â”€â”€â”€ Event delegation: product card clicks â”€â”€â”€ */
...
function _renderKategoriOptions(){
  ...
  const maxLabel=limit.k>=999999?"âˆž":limit.k;
  const sisaLabel=limit.k>=999999?"âˆž":sisa>0?sisa:"0 â€” limit";

  let html=`
    <div class="kpopup-quota">
      <div class="kpopup-quota-row"><span>ðŸ“¦ Kategori terpakai</span><b>${used} / ${maxLabel}</b></div>
    ...
    <div class="kategori-opt-item${currentKategori==="Semua"?" active":""}" data-kat="Semua">
      <span>ðŸ </span> Semua <span class="kopt-count">${products.length} produk</span>
    </div>`;

  if(!katSet.length){
    html+=`<div class="kpopup-empty"><div style="font-size:28px">ðŸ“‚</div><div>Belum ada kategori</div></div>`;
  } else {
    html+=katSet.map(k=>{
      const jumlah=products.filter(p=>(pField(p,"kategori")||"Umum")===k).length;
      return `<div class="kategori-opt-item${k===currentKategori?" active":""}" data-kat="${k}"><span>ðŸ·ï¸</span> ${k} <span class="kopt-count">${jumlah} produk</span></div>`;
    }).join("");
  }
...
function _renderCart(){
  ...
  if(!cart.length){
    cartList.innerHTML=`<div class="empty-state"><div class="empty-icon">ðŸ›’</div><p>Keranjang masih kosong</p></div>`;
    return;
  }
  ...
  cart.forEach(i=>{
    ...
    const harga=getHarga(real,products);
    const sub=harga*i.qty;
    const ph="https://placehold.co/56/1e2740/5cf17b?text=ðŸ›";
    const div=document.createElement("div");
  ...
  summaryDiv.innerHTML=`
    ...
    ${diskonOwner>0?`<div class="cart-summary-row green"><span>ðŸ·ï¸ Diskon Produk ${diskonOwner}%</span><span>-Rp ${diskonNominal.toLocaleString("id")}</span></div>`:""}
    ${diskonPesan>0?`<div class="cart-summary-row green"><span>ðŸŽ Diskon Pesan ${diskonPesan}%</span><span>-Rp ${Math.round(totalFinal*diskonPesan/100).toLocaleString("id")}</span></div>`:""}
    <div class="cart-summary-row"><span>ðŸšš Ongkir</span><span>${data.ongkir?"Rp "+data.ongkir.toLocaleString("id"):"<span class=\'free-badge\'>GRATIS</span>"}</span></div>
...
function checkout(){
  ...
  document.getElementById("checkoutTotal").innerHTML=`
    <div class="checkout-total">
      ...
      ${diskonPesan>0?`<div class="cart-total-row diskon-row"><span>ðŸŽ Diskon ${diskonPesan}%</span><span>-Rp ${Math.round(data.total*diskonPesan/100).toLocaleString("id")}</span></div>`:""}
      <div class="cart-total-row"><span>ðŸšš Ongkir</span><span>${data.ongkir?"Rp "+data.ongkir.toLocaleString("id"):"GRATIS ðŸŽ‰"}</span></div>
      <div class="cart-total-row cart-grand"><span>ðŸ’° Total Bayar</span><span>Rp ${grandTampil.toLocaleString("id")}</span></div>
      <div class="pay-nonce-row"><span>ðŸ” Kode unik akhir</span><span class="nonce-badge">#${nonce}</span></div>
...
async function buildOrderMessage(nama, waP, alamat, extraFields={}){
  ...
  cart.forEach(i=>{
    ...
    const harga = getHarga(real, products);
    itemLines  += `â€¢ ${pNama} (${i.qty}Ã—) = Rp ${(harga*i.qty).toLocaleString("id")}\n`;
  });

  let msg = `ðŸ›’ *PESANAN BARU â€” ${storeName}*\n`;
  msg    += `ðŸ“‹ Invoice: *${inv||"â€”"}*\n`;
  msg    += `ðŸ“… ${tgl}\n`;
  msg    += `\nðŸ‘¤ *${nama}*\nðŸ“± ${waP}\nðŸ“ ${alamat}\n`;
  msg    += `\nâ”â”â” PESANAN â”â”â”\n${itemLines}`;
  msg    += `\nâ”â”â” RINGKASAN â”â”â”\n`;
  msg    += `Subtotal  : Rp ${data.total.toLocaleString("id")}\n`;
  if(diskonPesan>0) msg += `ðŸŽ Diskon ${diskonPesan}%: -Rp ${Math.round(data.total*diskonPesan/100).toLocaleString("id")}\n`;
  if(cfg.diskonGlobal>0) msg += `ðŸ·ï¸ Diskon Produk: ${cfg.diskonGlobal}%\n`;
  if(data.bundleActive) msg += `ðŸ”¥ Bundle Deal: aktif\n`;
  msg    += `ðŸšš Ongkir  : ${data.ongkir?"Rp "+data.ongkir.toLocaleString("id"):"GRATIS"}\n`;
  msg    += `\nðŸ’° *TOTAL BAYAR: Rp ${grandTampil.toLocaleString("id")}*\n`;
  msg    += `ðŸ” Kode Bayar: *${sig}* | #${nonce}\n`;
  msg    += `\n_Tunjukkan kode ini kepada penjual sebagai bukti pesanan asli._\n`;
  msg    += `\nTerima kasih! ðŸ™`;
  return { msg, grandTampil, nonce, sig, inv };
}
...
async function sendEmail(){
  const email=localStorage.getItem("ownerEmail"); if(!email){showToast("âŒ Email owner belum diset");return;}
  const storeName=localStorage.getItem("storeName")||"Toko WA";
  const result=await buildOrderMessage(
  ...
  window.open(`mailto:${email}?subject=${encodeURIComponent("Pesanan Baru â€” "+storeName+" â€” "+new Date().toLocaleDateString("id"))}&body=${encodeURIComponent(result.msg)}`);
  saveCart([]); closeCheckout(); updateCartUI();
  showToast("âœ… Draft email dibuka");
  if(window.Admin) Admin.renderAkuntansi&&Admin.renderAkuntansi();
}

const chatWA=()=>{ const wa=localStorage.getItem("ownerWa"); if(!wa){showToast("âŒ No WA belum diset");return;} window.open("https://wa.me/"+wa); };
...
function addNewKategori(){
  ...
  const nama=input.value.trim(); if(!nama){showToast("âŒ Nama kosong");return;}
...
// openDevAccess removed â€” console-injectable method is a security risk.
// Dev mode is triggered via 00990099 + BACKSPACEÃ—3 keyboard sequence (core.js).
...
async function _posRenderGrid(){
  ...
  el.innerHTML = filtered.map(p=>{
    ...
    const imgHtml = img
      ? `<img src="${img}" loading="lazy" onerror="this.parentNode.innerHTML='ðŸ›ï¸'">`
      : `ðŸ›ï¸`;
...
/* â”€â”€ Cart render with full breakdown â”€â”€ */
...
/* â”€â”€ Customer fields: ENTER flow + auto-lookup â”€â”€ */
...
function _posShowSuccess(nama, grand, inv){
  ...
  div.innerHTML=`
    <div class="pos-success-icon">âœ…</div>
    <div class="pos-success-title">Transaksi Berhasil!</div>
    <div class="pos-success-sub">${nama} â€” Rp ${grand.toLocaleString("id")}</div>
...
function posClearCart(){
  ...
  _posRenderCart();
  _posClearFields();
  showToast("ðŸ—‘ï¸ Keranjang dikosongkan");
}

/* â”€â”€ Safe POS sync â”€â”€ */
function _posSyncUI(){
  if(document.getElementById("posView")?.style.display!=="none"){
    _posRenderGrid();
    _posRenderCart();
...
/* â”€â”€ POS event delegation (document-level) â”€â”€ */
document.addEventListener("click", e=>{
...
function _appInit(){
  ...
  console.log("APP v4.2 READY ðŸš€");
}

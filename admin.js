...
function _updateExistingProduct(id){
  ...
  if(pImgInput && pImgInput.files && pImgInput.files[0]){
    const reader = new FileReader();
    reader.onload = ev => {
      if(products[idx].g !== undefined) products[idx].g = ev.target.result;
      else products[idx].img = ev.target.result;
      saveProducts(products);
      renderOwnerProdukList();
      if(window.App) App.renderFull && App.renderFull();
    };
    reader.readAsDataURL(pImgInput.files[0]);
    return;
  }
  saveProducts(products);
...
function addProduct(){
  ...
  const newProduct = { i: Date.now(), n: name, p: price, m: modal, k: kat, g: img, stok: stok, sumber: sumber, tempo: tempo };

  function _resetForm(){
    const katEl = document.getElementById("pKategori");
    if(katEl) katEl.value = kat; // pertahankan kategori terakhir
    ["pName","pPrice","pModal","pStok"].forEach(id=>{ const el=document.getElementById(id); if(el) el.value=""; });
...
function renderOwnerProdukList(){
  ...
  for(; idx < end; idx++){
    ...
    const stokLabel = stokNum === null ? "\xe2\x80\x94" : stokNum <= 0 ? "Habis" : stokNum;
      ...
      div.innerHTML = `
        ...
        <div style="flex:1; text-align:center;">
          <select class="status-badge status-${status}" style="margin:0; padding:2px 6px;" onchange="Admin.updateTxStatus('${tx.inv||tx.id}',this.value,this)">
            <option value="paid"  ${status==="paid"  ?"selected":""}>\xe2\x9c\x85 Paid</option>
            <option value="wait"  ${status==="wait"  ?"selected":""}>\xe2\x8f\xb3 Wait</option>
            <option value="cancel"${status==="cancel"?"selected":""}>\xe2\x9d\x8c Cancel</option>
          </select>
        </div>
...
function renderHistory(){
  ...
  const rp = v => "Rp " + Math.round(v).toLocaleString("id");
  const sorted = txAll.slice().reverse();
...
function renderPelangganList(filter){
  ...
  filtered.forEach(([hp, c]) => {
    const initial = (c.nama||"?")[0].toUpperCase();
    ...
    div.innerHTML = `
      <div class="pelanggan-avatar">${initial}</div>
      <div class="pelanggan-info">
        <div class="pelanggan-name">${c.nama||"—"}</div>
        <div class="pelanggan-wa">${hp}</div>
        <div class="pelanggan-addr">📍 ${c.alamat||"—"}</div>
      </div>
      <div style="display:flex;flex-direction:column;gap:4px;align-items:flex-end">
        <div class="pelanggan-last">Terakhir: ${lastDate}</div>
        <button class="pelanggan-wa-btn" onclick="window.open('https://wa.me/${hp}')">💬 WA</button>
      </div>`;
    frag.appendChild(div);
  });
  el.innerHTML=""; el.appendChild(frag);
